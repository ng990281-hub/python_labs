...
Вводим переменные для имен.Выводим имя и к переменной возраста прибавляем один.
```python
a=input("Имя: ")
b=int(input("Возраст: "))
...
Вводную переменную разбить на 3 переменные. Чтобы вывести инициалы, нужно взять первые символы этих переменных(если символы не заглавные применить upper). Далее вводную переменную склеиваю и разбиваю  по пробелам.
```python
a=input("ФИО: ")
n,s,o=a.split()
m=(n[0]+s[0]+o[0]).upper()
